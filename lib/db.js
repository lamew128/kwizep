...
const getKwizResult = (quizTakeId) => {
  ...
  const getQuizResult = `SELECT users.name as user,score,quiz.url,title,description
                         FROM quiz_result
...
const generateKwizResponsee = (userId,kwizId,quizAnswers) => {
  // [q1B, q2A, q3D, q4A] => [100,123,143,150, 160]
  // [true,false,true,true]
  let answersChosen = [];
  pool.query(`SELECT quiz_answers.id as answerid, quiz_question.id as questionid, quiz_answers.answer_index as index
   FROM quiz_answers
   JOIN quiz_question ON quiz_question_id = quiz_question.id
   WHERE quiz_answers.quiz_id = $1
   ORDER BY quiz_question.id, quiz_answers.id`,[kwizId])
   .then(result => {
          console.log("resulttttttt", result);
          let allAnswers = result.rows;
          console.log('allAnswers',allAnswers);
          let startQuestion = result.rows[0].questionid;
          for (let answer of quizAnswers) {
            // console.log('answer is',answer);
            const index = answer[answer.length - 1];
            // console.log('index is', index);
            const questionNumber = Number(answer.slice(1,-1));
            const questionId = startQuestion + questionNumber - 1;
            console.log({questionId});
            let answerId;
            for (let answer of allAnswers) {
              if (answer.questionid == questionId && answer.index == index) {
...
// const generateKwizResponse2 = (userId,kwizId,quizAnswers) => {
//   // [q1B, q2A, q3D, q4A] => [100,123,143,150, 160]
//   // [true,false,true,true]
//   let answersChosen = [];
//   pool.query(`SELECT answer.id, question.id
//    FROM quiz_answers
//    JOIN quiz_questions ON quiz_question_id = question.id
//    WHERE quiz_answers.quiz_id = $1
//    ORDER BY quiz_question.id, quiz_answers.id`,[kwizId])
//    .then(result => {
//           console.log("resulttttttt", result);
