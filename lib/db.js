 ...
 * @return {Promise<{}>} A promise to the user.
 */
//  const addKwizAnswers = (quizId, questionId,correct,answer,option) => {
//   let KwizAnswerParams = [quizId, questionId,correct,answer,option];
//   console.log(KwizAnswerParams);
//   pool.query
...
//  };
 ...
 const addKwizAnswers = (id,answerArr) => {
  for (let i = 0; i < answerArr.length; i++){
      let kwizAnswersParams = [id, answerArr[i].questionId, answerArr[i].correct, answerArr[i].answer,answerArr[i].option];
      console.log(kwizAnswersParams)
      pool.query
      (`INSERT INTO quiz_answers (quiz_id, quiz_question_id, correct, answer, answer_index)
      VALUES ($1,$2,$3,$4,$5);`,
      kwizAnswersParams)
...
const showPublicKwizzes = () => {
  return pool
    .query(
      `Select id, title, description, url as imageurl, public FROM quiz
       WHERE public = true;`)
    .then((result) => {
