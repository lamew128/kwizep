...
const getKwizCover = function(kwizId) {
  return pool
    .query(
      `SELECT title, description, url FROM quiz
       WHERE id = $1`, [kwizId])
    .then((result) => {
      return result.rows[0];
    })
...
const myLastKwiz = (userId) => {
  const showMyLastKwiz = `SELECT score, title
                          FROM quiz_result
                          JOIN quiz_take ON quiz_result.quiz_take_id = quiz_take.id
                          JOIN quiz ON quiz_take.quiz_id = quiz.id
                          WHERE quiz_result.taker_id = $1
