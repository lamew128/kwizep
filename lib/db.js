...
const takeKwiz = (kwizId) => {
  ...
  return pool
   .query(
     ...
     `SELECT quiz_answers.id,quiz_question.question
      FROM quiz_answers
      JOIN quiz_question ON quiz_answers.quiz_id = quiz_question.quiz_id
      WHERE quiz_answers.quiz_id = 90
   ...
   .then((result) => {
     const quizObj = {};
     quizObj.kwizId = {};
     for (let i = question; i < question.length; i++){
